...
```{r fig.width=12}
qtl_tar = qtl2::scan1(genoprobs = subset(genoprobs, chr = chr_id),
                      pheno = pheno_data[,target_name,drop=FALSE], 
            kinship = kinship, addcovar = addcovar, intcovar = intcovar, cores = 2)
autoplot(qtl_tar, map, main = target_name) +
...
```{r fig.width=12}
qtl_med = qtl2::scan1(genoprobs = subset(genoprobs, chr = chr_id),
                      pheno = mrna$expr[, mrna_id, drop = FALSE], 
            kinship = kinship, addcovar = addcovar, intcovar = intcovar, cores = 2)
autoplot(qtl_med, map, main = mediator_name) +
  geom_vline(xintercept = mrna_pos, col = "grey")
```

It is useful to look at the genome scan of target adjusted by the mediator, to see that most of the signal is removed.

```{r}
m <- match(rownames(addcovar), rownames(mrna$expr), nomatch = 0)
medcovar <- cbind(addcovar[m>0,, drop = FALSE],
                  mrna$expr[m, mrna_id, drop = FALSE])
```

```{r fig.width=12}
qtl_tar_med = qtl2::scan1(genoprobs = subset(genoprobs, chr = chr_id),
                      pheno = pheno_data[,target_name,drop=FALSE], 
            kinship = kinship,
            addcovar = medcovar, intcovar = intcovar, cores = 2)
autoplot(qtl_tar_med, map, main = paste(target_name, "given", mediator_name)) +
  geom_vline(xintercept = pos_Mbp, col = "grey")
```

```{r}
# Relationship $T\vert M$
t.m <- qtl2::fit1(cbind(1, as.matrix(mrna$expr[, mrna_id, drop = FALSE])),
                  pheno_data[,target_name,drop=FALSE],
                  kinship,
                  addcovar)
...
```{r}
# Relationship c
m.t <- qtl2::fit1(cbind(1, as.matrix(pheno_data[,target_name,drop=FALSE])),
                  mrna$expr[, mrna_id, drop = FALSE],
                  kinship,
                  addcovar)$lod
```
...
```{r}
qtl_cmst <- cbind(qtl_med, qtl_tar, qtl_tar, qtl_tar_med, qtl_tar)
colnames(qtl_cmst) <- c("causal","reactive","independent","undecided","mediation")
...
```{r}
med_med <- intermediate::mediation_test(target = pheno_data[,target_name,drop=FALSE],
         mediator = mrna$expr[, mrna_id, drop = FALSE],
...
```{r fig.width=12}
med_triad <- intermediate::mediation_triad(target = pheno_data[,target_name,drop=FALSE],
         mediator = mrna$expr[, mrna_id, drop = FALSE],
         driver = genoprobs[[chr_id]][,,mrna_driver],
         sdp = NULL,
         covar_tar = addcovar,
...
assoc_tar = qtl2::scan1snps(genoprobs = pairprobs[,chr_id], map = pairmap, 
                      pheno = pheno_data[,target_name,drop=F],
                      kinship = kinship,
...
assoc_tar_med = qtl2::scan1snps(genoprobs = pairprobs[,chr_id], map = pairmap, 
                      pheno = pheno_data[,target_name,drop=F],
                      kinship = kinship,
                      addcovar = medcovar, intcovar = intcovar, chr = chr_id, start = start,
                      end = end, query_func = query_variant, cores = 4,
                      keep_all_snps = FALSE)
```
...
```{r}
mediator <- mrna$expr[, rep(mrna_id, dim(sp)[3]), drop = FALSE]
colnames(mediator) <- dimnames(sp)[[3]]
```

```{r}
m <- match(mrna_id, mrna.annot$id)
annotation <- mrna.annot[rep(m, ncol(mediator)),]
...
```{r fig.width=12}
med_snp_triad <- intermediate::mediation_triad(target = pheno_data[,target_name,drop=FALSE],
         mediator = mrna$expr[, mrna_id, drop = FALSE],
