...
It is helpful to examine the SNPs organized by their SNP distribution pattern, looking at only the top one or two patterns. This looks a lot more like the genome allele scan.
...
(tar_best <- bind_rows(lapply(tar_test[peak_snps], summary), .id = "snp_id") %>%
  mutate(source = c("target","mediator","causal"),
         pos_snp = pos_snps) %>%
  select(source, symbol,chr, pos_snp, snp_id, pvalue, LR, triad, mediation, everything())) %>%
  select(source, symbol,chr,pos_snp, snp_id, pvalue,triad,mediation,LR) %>%
  rename(lod = "LR") %>%
  mutate(lod = lod / log(10),
         mediation = mediation / log(10))
```
